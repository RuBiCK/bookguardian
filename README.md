...
| `GET`    | `/api/lookup/search?q=&title=&author=&isbn=&publisher=&year=&limit=` | `{ items: (BookDraft & { resultId })[] }` — free text and/or structured fields (any one is enough). Both providers are asked at once; same ISBN-13 appears once, exact ISBN first, then title + author matches. Hits with an ISBN land in `catalog_books`. Limit 1–10, default 5 |
...
### Add book → Search online (web)

The manual add form can do the typing for you: fill in whatever is known —
title, authors, ISBN, publisher, year, any one field is enough — and tap
**Search online** (Enter from one of those fields does the same). A bottom
sheet lists up to ten matches with cover, title, authors, publisher · year,
ISBN-13 and the provider; a tap fills the form. Merge rules
(`mergeDraftIntoForm` in `apps/web/src/lib/book-form.ts`): empty fields take
the online value, fields you typed are kept and get a **Use online value: …**
chip when the result differs, the ISBN always comes from the result (it is a
key, not an opinion) and the cover arrives through the usual cascade by ISBN
(or from the provider image URL when the result has no ISBN). Leaving the
sheet or editing a field cancels a search still running; nothing is remembered
between searches.
